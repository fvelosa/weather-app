...
var isBuild = !!(process.argv.filter(function (val, index, array) {
	return val === '-p';
}).length && exttractCssOnBuild);
...
module.exports = {
	...
	module: {
		loaders: [
			...
			{
				test: /\.scss$/,
				loader: isBuild ? ExtractTextPlugin.extract('style', 'css?sourceMap!autoprefixer?browsers=last 2 version!sass?outputStyle=expanded&sourceMap') : 'style!css?sourceMap!autoprefixer?browsers=last 2 version!sass?outputStyle=expanded&sourceMap'
            },
			{
				test: /\.css$/,
			...
			{
				test: /\.(woff)(\?.*$|$)/,
				loader: 'url?name=[hash].[ext]limit=10000&mimetype=application/font-woff'
			},
			{
				test: /\.(woff2)(\?.*$|$)/,
				loader: 'url?name=[hash].[ext]&limit=10000&mimetype=application/font-woff2'
			},
			{
				test: /\.(ttf)(\?.*$|$)/,
				loader: 'url?name=[hash].[ext]limit=10000&mimetype=application/octet-stream'
			},
			{
				test: /\.(eot)(\?.*$|$)/,
				loader: 'file?name=[hash].[ext]?[param]'
			},
			{
				test: /\.(svg)(\?.*$|$)/,
